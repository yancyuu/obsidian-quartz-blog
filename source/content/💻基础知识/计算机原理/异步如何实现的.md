...
### 深入：为什么 CPU 密集会阻塞事件循环

事件循环本质是**单线程 + 非抢占式调度**：一个任务必须**完整执行完**，事件循环不会中途打断它去干别的。

- **IO 不阻塞**：`readFile`/网络请求把活**外包给 libuv/OS**，主线程立即返回不等，完成后靠事件通知，回调下一轮处理。
- **CPU 阻塞**：CPU 计算（`while`、大运算、`JSON.parse` 大对象）**本身就是代码，必须主线程亲自执行**，无法外包 → 独占线程 → 事件循环卡死 → 其他所有回调堵住 → 进程假死。

> 一句话：**IO 是甩活给别人（异步），CPU 是自己必须干完（同步独占）**。
> 解决：Node 用 `worker_threads`、Python 用 `multiprocessing`/`run_in_executor`——本质都是「把 CPU 活挪出主线程」；或拆分任务用 `setImmediate`/分块让出。
